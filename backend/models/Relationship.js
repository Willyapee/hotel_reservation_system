...
const defineRelationships = () => {
    ...
    Reservations.hasOne(Invoices, {              // ← KOREKSI: Invoices
        foreignKey: 'id_reservation',
        as: 'invoice',
    });
    Invoices.belongsTo(Reservations, {           // ← KOREKSI: Invoices
        foreignKey: 'id_reservation',
        as: 'reservation',
    });

    // 3. INVOICE - PAYMENT (One-to-One)
    Invoices.hasOne(Payments, {                  // ← KOREKSI: Invoices & Payments
        foreignKey: 'id_invoice',
        as: 'payment',
    });
    Payments.belongsTo(Invoices, {               // ← KOREKSI: Invoices & Payments
        foreignKey: 'id_invoice',
        as: 'invoice',
    });
