...
export default function NavigationBar({ openMenu, handleOpenMenu }) {
	return (
		<div className='w-full h-fit fixed flex items-center gap-x-10 px-4 py-2 bg-[#102E50] z-1'>
			<img src='../picture/logo/logo.png' className='w-13 h-fit' />
			<Link to={openMenu ? '/' : '/menu'}>
				<button onClick={handleOpenMenu} className='gap-x-2 flex items-center text-white'>
					<span>{openMenu ? '✕' : '☰'}</span>
